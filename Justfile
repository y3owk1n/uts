...
release-ci VERSION_OVERRIDE:
	...
	env GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-s -w -X github.com/y3owk1n/uts/cmd.Version={{ VERSION_OVERRIDE }}" -trimpath -o build/uts-windows-amd64.exe ./main.go
	env GOOS=windows GOARCH=arm64 CGO_ENABLED=0 go build -ldflags="-s -w -X github.com/y3owk1n/uts/cmd.Version={{ VERSION_OVERRIDE }}" -trimpath -o build/uts-windows-arm64.exe ./main.go
